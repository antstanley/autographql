...
const loadDefault = () => {
  if (existsSync(join(projectRoot, './faasql.config.json'))) {
    return JSON.parse(readFileSync(join(projectRoot, './faasql.config.json'), 'utf-8'));
  } else {
    if (existsSync(join(projectRoot, './faasql.config.yml'))) {
      return YAML.parse(readFileSync(join(projectRoot, './faasql.config.yml'), 'utf-8'));
    } else {
      logger('warn', 'No faasql.config.json or faasql.config.yml found, using defaults.');
      return null;
    }
  }
};
