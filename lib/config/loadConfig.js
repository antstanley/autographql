...
const loadDefault = () => {
  if ((0, _fs.existsSync)((0, _path.join)(projectRoot, './faasql.config.json'))) {
    return JSON.parse((0, _fs.readFileSync)((0, _path.join)(projectRoot, './faasql.config.json'), 'utf-8'));
  } else {
    if ((0, _fs.existsSync)((0, _path.join)(projectRoot, './faasql.config.yml'))) {
      return _yaml.default.parse((0, _fs.readFileSync)((0, _path.join)(projectRoot, './faasql.config.yml'), 'utf-8'));
    } else {
      (0, _utils.logger)('warn', 'No faasql.config.json or faasql.config.yml found, using defaults.');
      return null;
    }
  }
};
