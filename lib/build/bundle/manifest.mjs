...
(function () {
  var _ref = _asyncToGenerator(function* (rootDir) {
    var functionsDir = rootDir + "/build/functions";
    var rootArray = readdirSync(functionsDir);
    if (rootArray.length <= 0) throw new Error('Invalid root path');
    var manifestArray = [];
    var i;

    for (i = 0; i < rootArray.length; i++) {
      var functionRoot = functionsDir + "/" + rootArray[i]; // console.log(`iteration: ${i}, root: ${functionRoot}`)
