...
const copyFunc = async ({
  ...
  try {
    (0, _utils.logger)('info', `${provider.name}: Copying function to distribution folder '${provider.dist}'`);
    ...
    if ((0, _fs.existsSync)(fileDestDir)) {
      ...
      (0, _fs.mkdirSync)(fileDestDir);
      (0, _fs.copyFileSync)(fileSrc, fileDest);
      return true;
    }
  } catch (error) {
    (0, _utils.logger)('warn', `Unable to copy function to location with error: ${error}`);
    return false;
  }
};
